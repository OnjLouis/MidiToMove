...
function Info([string]$message) { Write-Host "[MidiToMove release] $message" }
...
function Get-GitHubHeaders {
    $headers = @{ 'User-Agent' = 'MidiToMove release check'; 'Accept' = 'application/vnd.github+json' }
...
function Assert-CleanSourceTree {
    foreach ($name in @('MidiToMove.ini', 'MidiToMove failures.log', 'token.txt')) {
...
function New-ReleasePackage([string]$version) {
    ...
    $builtExe = Join-Path $portableDir 'MidiToMove.exe'
    ...
    $zipPath = Join-Path $stageDir ("MidiToMove-$version-portable.zip")
...
"@
    try {
        ...
        $body = @{ tag_name = $tag; name = "MidiToMove $version"; body = $releaseBody; draft = $false; prerelease = $false } | ConvertTo-Json
